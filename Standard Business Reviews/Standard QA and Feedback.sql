...
Declare @iDME as int, @YYYY int, @MM int, @Span int, @xDate as date, @sDate as datetime, @eDate as char(10) ,@mm_name char(15)
--set @YYYY = 2016 --Last Year and Month of the data to collect (2013-12 = December as Last month of X Span months)
select @YYYY=format(year(getdate()),'0000')
select @MM = format(month(getdate())-1,'00')
select @mm_name=DATENAME(MONTH, GETDATE()) -- TO correct the logic for month-1


--The following if condition runs for January and Corrects the Month and year  
if @MM=0  begin
	select @YYYY=format(year(getdate())-1,'0000')
	set @mm=12
	select @mm_name=DATENAME(MONTH, GETDATE()-32) -- TO correct the logic for month-1
end
...
print @xDate
print @eDate
Declare @iHOS as int,@loop_step int,@loopdate date


IF OBJECT_ID('reports_bic.dbo.iHOS_listBizReview', 'U') IS NULL 
begin
declare @table_text char(100)
set @table_text = 'Hospice List Table Not present. Creating the table.... '
print @table_text

--  DROP TABLE reports_bic.dbo.iHOS_listBizReview; 

create table reports_bic.dbo.iHOS_listBizReview(
iHOS_SEQ_NO int,
iHOS_ID int,
...
--insert into reports_bic.dbo.iHOS_listBizReview values(sequence_number,hospice id, Hospice Name)

insert into reports_bic.dbo.iHOS_listBizReview values(1,10,'Optum Palliative and Hospice Care') --Optum Palliative and Hospice Care
insert into reports_bic.dbo.iHOS_listBizReview values(2,12,'Alacare Home Health & Hospice')  --Alacare Home Health & Hospice
insert into reports_bic.dbo.iHOS_listBizReview values(3,257,'Harbor Light Hospice') --Harbor Light Hospice
insert into reports_bic.dbo.iHOS_listBizReview values(4,238,'SouthernCare, Inc {Curo}') --SouthernCare, Inc {Curo}
insert into reports_bic.dbo.iHOS_listBizReview values(5,187,'Hospice of Santa Cruz County') --Hospice of Santa Cruz County
insert into reports_bic.dbo.iHOS_listBizReview values(6,269,'Suncrest Health Services') --Suncrest Health Services
insert into reports_bic.dbo.iHOS_listBizReview values(7,1057,'Sta-Home Health & Hospice, Inc.') --Sta-Home Health & Hospice, Inc.
insert into reports_bic.dbo.iHOS_listBizReview values(8,120,'Banner Hospice') --Banner Hospice
insert into reports_bic.dbo.iHOS_listBizReview values(9,232,'Hospice Partners of America') --Hospice Partners of America
insert into reports_bic.dbo.iHOS_listBizReview values(10,295,'Encompass Home Health & Hospice') 
insert into reports_bic.dbo.iHOS_listBizReview values(11,227,'Delaware Hospice')-- Delaware Hospice
insert into reports_bic.dbo.iHOS_listBizReview values(12,144,'Signature Hospice')-- Signature Hospice 

end

 IF OBJECT_ID('reports_bic.dbo.StandardregionalQuestions', 'U') IS NOT NULL 
  DROP TABLE reports_bic.dbo.StandardregionalQuestions; 

create table reports_bic.dbo.StandardregionalQuestions
( Question varchar(50) Not null,
  ...
  IF OBJECT_ID('reports_bic.[dbo].[StandardFeedback]', 'U') IS NOT NULL 
  DROP TABLE reports_bic.[dbo].[StandardFeedback]; 
  
  CREATE TABLE reports_bic.[dbo].[StandardFeedback](
	[iOID] [int] NOT NULL,
...
IF OBJECT_ID('reports_bic.dbo.StandardNationalQuestions', 'U') IS NOT NULL 
  DROP TABLE reports_bic.dbo.StandardNationalQuestions; 

create table reports_bic.dbo.StandardNationalQuestions
( Question varchar(50) Not null,
  meet numeric(9,6) NULL,
  fail numeric(9,6),
  exceed numeric(9,6),
 ...
 IF OBJECT_ID('reports_bic.dbo.StandardClientQuestions', 'U') IS NOT NULL 
  DROP TABLE reports_bic.dbo.StandardClientQuestions; 

create table reports_bic.dbo.StandardClientQuestions
( Question varchar(50) Not null,
  meet numeric(9,6) NULL,
  fail numeric(9,6),
  exceed numeric(9,6),
  ...
  IF OBJECT_ID('reports_bic.dbo.TempStndRegional', 'U') IS NOT NULL 
  DROP TABLE reports_bic.dbo.TempStndRegional; 

create table  reports_bic.dbo.TempStndRegional (
	[iHOS] [int] NULL,
	[iRgn] [int] NULL,
	[iTotal] [int] NULL,
	[iNoCnt] [int] NULL,
...
IF OBJECT_ID('reports_bic.dbo.TempStndClient', 'U') IS NOT NULL 
  DROP TABLE reports_bic.dbo.TempStndClient; 

create table  reports_bic.dbo.TempStndClient (
	[iHOS] [int] NULL,
	[iRgn] [int] NULL,
	[iTotal] [int] NULL,
	[iNoCnt] [int] NULL,
...
Declare @iHOSCount as int , @iHOS_step as int , @iHOS_ID as int,@iHOS_SEQ_NO as int,@iHOSNAME as varchar(100)
select @iHOScount= count(*) from reports_bic.dbo.iHOS_listBizReview
print @iHOScount

select * from reports_bic.dbo.iHOS_listBizReview
set @iHOS_step=1

while (@iHOS_step <= @iHOScount)
begin

select @iHOS=iHOS_ID from reports_bic.dbo.iHOS_listBizReview where iHOS_SEQ_NO = @iHOS_step -- read Hospice id from the list 
select @iHOSNAME=iHOS_name from reports_bic.dbo.iHOS_listBizReview where iHOS_SEQ_NO = @iHOS_step
-----------
...
declare @Dyad as table (
	...
	select distinct r.isetid as iRegion, isnull(rtrim(r.sName),c.sState + ' Region') as sRegion,
		dy.iClientRef as iCID, ltrim(rtrim(c.sName)) as sClient,
...
select 'R', 20, iRgn, sRgn, null, 'Many' as sCName, null, 'Many' as sSName, null, 'Many' as sDMEName, rtrim(@iHOS), iRgn,
...
select 'H', 10, 0 as iRgn, 'All' as sRgn, null, 'All' as sCName, null, 'All' as sSName, null, 'All' as sDMEName, rtrim(@iHOS), 0,
	sum(isnull(iTotal,0)), sum(isnull(iNoCnt,0)), sum(isnull(iOpOut,0)), sum(isnull(iNoEng,0)), 
	sum(isnull(iAbort,0)), sum(isnull(iUsable,0)), sum(isnull(iDone,0)),
	sum(isnull(xQ1,0)), sum(isnull(iQ1y,0)), sum(isnull(iQ1n,0)), 0 as pQ1,
	sum(isnull(xQ2,0)), sum(isnull(iQ2y,0)), sum(isnull(iQ2n,0)), 0 as pQ2,
...
--get Notes
  ...
  insert into reports_bic.dbo.StandardNationalQuestions values ('Equipment',@pq1,1-@pq1,null,@smonth,@iHOSNAME);
  insert into reports_bic.dbo.StandardNationalQuestions values ('Instruction',@pq2,1-@pq2,null,@smonth,@iHOSNAME);
  insert into reports_bic.dbo.StandardNationalQuestions values ('Delivery Person',@pQ4m, @pQ4f, @pQ4x,@smonth,@iHOSNAME);
  insert into reports_bic.dbo.StandardNationalQuestions values ('Phone Service',@pQ5m, @pQ5f, @pQ5x,@smonth,@iHOSNAME);
  insert into reports_bic.dbo.StandardNationalQuestions values ('Timely Delivery',@pQ6m, @pQ6f, @pQ6x,@smonth,@iHOSNAME);
  insert into reports_bic.dbo.StandardNationalQuestions values ('Timely Pickup',@pQ7m, @pQ7f, @pQ7x,@smonth,@iHOSNAME);
...
insert into  reports_bic.dbo.TempStndRegional
select iHC_ID as iHOS, iSR_ID as iRgn, 
	iTotal, iNoCnt, iOpOut, iNoEng, iAbort, iUsable, iDone,
	xQ1, iQ1y, iQ1n, pQ1,
	xQ2, iQ2y, iQ2n, pQ2,
...
insert into  reports_bic.dbo.TempStndClient
select iHC_ID as iHOS, iSR_ID as iRgn, 
	iTotal, iNoCnt, iOpOut, iNoEng, iAbort, iUsable, iDone,
	xQ1, iQ1y, iQ1n, pQ1,
	xQ2, iQ2y, iQ2n, pQ2,
...
insert into reports_bic.[dbo].[StandardFeedback]
select QAR.iOrderRef as iOID, --QAF.cType, 
	QAFT.sName as sType, DATENAME(month ,QAF.dCreated) as dGiven,
	ss.fn_Qts_Show(QAF.sNote) as sFeedback,@iHOSNAME as HospiceName, --Remove the markup
	rtrim(isnull(re.sName,c.sState + ' Region')) as RegionName, rtrim(c.sName) as ClientName
...
IF OBJECT_ID('reports_bic.dbo.StandardRegional', 'U') IS NOT NULL 
  DROP TABLE reports_bic.dbo.StandardRegional; 

create table  reports_bic.dbo.StandardRegional (
	[seqno] [bigint] NULL,
	[iHOS] [int] NULL,
	[iRgn] [int] NULL,
	[iTotal] [int] NULL,
...
select 'Region Level' as info
insert into  reports_bic.dbo.StandardRegional
select ROW_NUMBER() over (order by Provider) as seqno,rtrim(iHOS),iRgn, 
	iTotal, iNoCnt, iOpOut, iNoEng, iAbort, iUsable, iDone,
	xQ1, iQ1y, iQ1n, pQ1,
	xQ2, iQ2y, iQ2n, pQ2,
	xQ4, iQ4m, iQ4x, iQ4f, pQ4m, pQ4x, pQ4f,
	...
	Client, Provider,
	dtActualBeginDate,	dtActualEndDate 
from reports_bic.dbo.TempStndRegional
--get Notes

declare @regcnt as int,@rgn_step as int,@regionname varchar(100)

select @regcnt=count(*) from reports_bic.dbo.StandardRegional
...
declare @providername varchar(100)
	...
	select @gmonth=DATENAME(month ,dtActualBeginDate) from reports_bic.dbo.StandardRegional where seqno=@rgn_step
	select @providername=  Provider from reports_bic.dbo.StandardRegional where seqno=@rgn_step


  select @perQ1 = pQ1 from reports_bic.dbo.StandardRegional where seqno=@rgn_step 
  select @perQ2 = pQ2 from reports_bic.dbo.StandardRegional where seqno=@rgn_step 
  select @perQ4m = pQ4m from reports_bic.dbo.StandardRegional where seqno=@rgn_step 
  select @perQ4x = pQ4x from reports_bic.dbo.StandardRegional where seqno=@rgn_step 
  select @perQ4f = pQ4f from reports_bic.dbo.StandardRegional where seqno=@rgn_step 
  select @perQ5m = pQ5m from reports_bic.dbo.StandardRegional where seqno=@rgn_step 
  select @perQ5x = pQ5x from reports_bic.dbo.StandardRegional where seqno=@rgn_step 
  select @perQ5f = pQ5f from reports_bic.dbo.StandardRegional where seqno=@rgn_step 
  select @perQ6m = pQ6m from reports_bic.dbo.StandardRegional where seqno=@rgn_step 
  select @perQ6x = pQ6x from reports_bic.dbo.StandardRegional where seqno=@rgn_step 
  select @perQ6f = pQ6f from reports_bic.dbo.StandardRegional where seqno=@rgn_step 
  select @perQ7m = pQ7m from reports_bic.dbo.StandardRegional where seqno=@rgn_step 
  select @perQ7x = pQ7x from reports_bic.dbo.StandardRegional where seqno=@rgn_step 
  select @perQ7f = pQ7f from reports_bic.dbo.StandardRegional where seqno=@rgn_step 



  insert into reports_bic.dbo.StandardregionalQuestions values ('Equipment',@perQ1,1-@perQ1,null,@gmonth,@providername,@iHOSNAME);
  insert into reports_bic.dbo.StandardregionalQuestions values ('Instruction',@perQ2,1-@perQ2,null,@gmonth,@providername,@iHOSNAME);
  insert into reports_bic.dbo.StandardregionalQuestions values ('Delivery Person',@perQ4m, @perQ4f, @perQ4x,@gmonth,@providername,@iHOSNAME);
  insert into reports_bic.dbo.StandardregionalQuestions values ('Phone Service',@perQ5m, @perQ5f, @perQ5x,@gmonth,@providername,@iHOSNAME);
  insert into reports_bic.dbo.StandardregionalQuestions values ('Timely Delivery',@perQ6m, @perQ6f, @perQ6x,@gmonth,@providername,@iHOSNAME);
  insert into reports_bic.dbo.StandardregionalQuestions values ('Timely Pickup',@perQ7m, @perQ7f, @perQ7x,@gmonth,@providername,@iHOSNAME);
...
IF OBJECT_ID('reports_bic.dbo.StandardClient', 'U') IS NOT NULL 
  DROP TABLE reports_bic.dbo.StandardClient; 

create table  reports_bic.dbo.StandardClient (
	[seqno] [bigint] NULL,
	[iHOS] [int] NULL,
	[iRgn] [int] NULL,
	[iTotal] [int] NULL,
...
select 'Standard Client Level' as info
insert into  reports_bic.dbo.StandardClient
select ROW_NUMBER() over (order by Client) as seqno,rtrim(iHOS),iRgn, 
	iTotal, iNoCnt, iOpOut, iNoEng, iAbort, iUsable, iDone,
	xQ1, iQ1y, iQ1n, pQ1,
	xQ2, iQ2y, iQ2n, pQ2,
	xQ4, iQ4m, iQ4x, iQ4f, pQ4m, pQ4x, pQ4f,
	...
	rtrim(Client), Provider,
	dtActualBeginDate,	dtActualEndDate 
from reports_bic.dbo.TempStndClient
--get Notes

declare @clientcnt as int,@client_step as int

select @clientcnt=count(*) from reports_bic.dbo.StandardClient
...
declare @clientname varchar(100)
	...
	select @gcmonth=DATENAME(month ,dtActualBeginDate) from reports_bic.dbo.StandardClient where seqno=@client_step
	select @cprovidername=  Provider from reports_bic.dbo.StandardClient where seqno=@client_step
	select @clientname= client from reports_bic.dbo.StandardClient where seqno=@client_step


  select @percQ1 = pQ1 from reports_bic.dbo.StandardClient where seqno=@client_step 
  select @percQ2 = pQ2 from reports_bic.dbo.StandardClient where seqno=@client_step 
  select @percQ4m = pQ4m from reports_bic.dbo.StandardClient where seqno=@client_step 
  select @percQ4x = pQ4x from reports_bic.dbo.StandardClient where seqno=@client_step 
  select @percQ4f = pQ4f from reports_bic.dbo.StandardClient where seqno=@client_step 
  select @percQ5m = pQ5m from reports_bic.dbo.StandardClient where seqno=@client_step 
  select @percQ5x = pQ5x from reports_bic.dbo.StandardClient where seqno=@client_step 
  select @percQ5f = pQ5f from reports_bic.dbo.StandardClient where seqno=@client_step 
  select @percQ6m = pQ6m from reports_bic.dbo.StandardClient where seqno=@client_step 
  select @percQ6x = pQ6x from reports_bic.dbo.StandardClient where seqno=@client_step 
  select @percQ6f = pQ6f from reports_bic.dbo.StandardClient where seqno=@client_step 
  select @percQ7m = pQ7m from reports_bic.dbo.StandardClient where seqno=@client_step 
  select @percQ7x = pQ7x from reports_bic.dbo.StandardClient where seqno=@client_step 
  select @percQ7f = pQ7f from reports_bic.dbo.StandardClient where seqno=@client_step 



  insert into reports_bic.dbo.StandardClientQuestions values ('Equipment',@percQ1,1-@percQ1,null,@gcmonth,@cprovidername,@clientname);
  insert into reports_bic.dbo.StandardClientQuestions values ('Instruction',@percQ2,1-@percQ2,null,@gcmonth,@cprovidername,@clientname);
  insert into reports_bic.dbo.StandardClientQuestions values ('Delivery person',@percQ4m, @percQ4f, @percQ4x,@gcmonth,@cprovidername,@clientname);
  insert into reports_bic.dbo.StandardClientQuestions values ('Phone Service',@percQ5m, @percQ5f, @percQ5x,@gcmonth,@cprovidername,@clientname);
  insert into reports_bic.dbo.StandardClientQuestions values ('Timely Delivery',@percQ6m, @percQ6f, @percQ6x,@gcmonth,@cprovidername,@clientname);
  insert into reports_bic.dbo.StandardClientQuestions values ('Timely Pickup',@percQ7m, @percQ7f, @percQ7x,@gcmonth,@cprovidername,@clientname);
