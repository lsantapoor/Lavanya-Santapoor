...
where cScope = 'N' and cPyramid = 'N'
 ...
 IF OBJECT_ID('reports_bic.dbo.ElizabethTop5Items', 'U') IS NOT NULL 
  DROP TABLE reports_bic.dbo.ElizabethTop5Items;
CREATE TABLE reports_bic.dbo.ElizabethTop5Items(
	[Contract] [varchar](15) NOT NULL,
	[Scope] [varchar](30) NOT NULL,
	[Report] [varchar](8) NOT NULL,
	[Region Name] [nvarchar](149) NULL,
...
insert into reports_bic.dbo.ElizabethTop5Items
--Get National Scope top 5 data by Contract Type.
select case a.cType 
		when 'B&M' then 'B&M'
		when 'NET' then 'NET'
	...
	'National by Region' as 'Scope', '% of FFS' as 'Report',
	a.sRgn + '   (' + CAST(a.iRgn as varchar(16)) + ')' as 'Region Name (r)', 
	a.sItemName + '   (' + CAST(a.iItemID as varchar(16)) + ')' as 'Top 5 Item (i)', 
	isnull(r.m1Per,0) as m1Per, isnull(r.m2Per,0) as m2Per, isnull(r.m3Per,0) as m3Per
from (
	--All Items on All Regions.
	select n.cType, r.iRgn, r.sRgn, n.iItemID, n.sItemName, n.mXTtl
	from (
		--National Item List
		select cType, iItemID, sItemName, mXTtl
		from @Rollup 
		where cScope = 'N' and cPyramid = 'N'
	) as n
	join (
		--Bounced into Region List
		select distinct cType, iRgn, sRgn from @Dyad where cType is not null
	) r on n.cType = r.cType --and n.iItemID = r.iItemID 
) as a
left join (
	--Get National Scope top 5 data by Contract Type (this will have holes if not at region level).
	select cType, iRgn, sRgn, iItemID, m1Per, m2Per, m3Per
	from @Rollup 
	where cScope = 'N' and cPyramid = 'R'
) as r on a.cType = r.cType and a.iRgn = r.iRgn and a.iItemID = r.iItemID 
order by a.cType, a.sRgn, a.mXTtl desc, a.sItemName 
