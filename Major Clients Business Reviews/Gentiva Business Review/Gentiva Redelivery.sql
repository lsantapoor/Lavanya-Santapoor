...
6 -  
*/
 IF OBJECT_ID('reports_bic.[dbo].[GentivaRedelivery]', 'U') IS NOT NULL 
  DROP TABLE reports_bic.[dbo].[GentivaRedelivery]; 


CREATE TABLE reports_bic.[dbo].[GentivaRedelivery](
    smonth [varchar](12),
...
\*-----------------------------------*/
	insert into reports_bic.dbo.GentivaRedelivery
	select @smonth,rtrim(d.sName + ', ' + (s.sName)) as Provider, rtrim(ltrim(c.sName)) as Client, rtrim(isnull(re.sName,c.sState + ' Region')) as RegionName, --DATENAME(month ,pd.dComplete) as imonth
