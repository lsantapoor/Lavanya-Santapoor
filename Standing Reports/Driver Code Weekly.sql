...
IF OBJECT_ID('reports_bic.dbo.UserSecurity_sorted_Wkly', 'U') IS NOT NULL 
  DROP TABLE reports_bic.dbo.UserSecurity_sorted_Wkly; 

  IF OBJECT_ID('reports_bic.dbo.alldriverdata_Wkly', 'U') IS NOT NULL 
  DROP TABLE  reports_bic.dbo.alldriverdata_Wkly; 

  CREATE TABLE reports_bic.[dbo].[alldriverdata_Wkly](
...
select * into reports_bic.dbo.UserSecurity_sorted_Wkly from dbo.UserSecurity where SecurityId = 17
order by Userid ASC
...
select @DriverUserId= min(Userid) from reports_bic.dbo.UserSecurity_sorted_Wkly -- read min Driver id from the list 
...
--check to make sure the temp table doesn't already exist
	...
	insert into reports_bic.dbo.alldriverdata_Wkly select [DateCompleted],
	[SiteId],
	[SiteName] ,
	[DriverId] ,
	[DriverName] ,
...
delete from reports_bic.dbo.UserSecurity_sorted_Wkly where Userid=@DriverUserId
...
set @Drv_step = @Drv_step +1
 ...
 Declare @query nvarchar(max);
    Set @query = 'select * from reports_bic.dbo.alldriverdata_Wkly';
 ...
 Execute msdb.dbo.sp_send_dbmail
	 ...
	 ,@copy_recipients = 'lsantapoor@stateserv.com'
      , @subject = 'Driver Accuracy Report Weekly'
      , @body = 'Hi,
	  
	  Please find the weekly Driver Accuracy Report attached.
      ...
      , @query_attachment_filename = 'Driver Accuracy Weekly.csv'
