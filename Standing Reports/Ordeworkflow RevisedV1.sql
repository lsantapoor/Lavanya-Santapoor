...
/****** Object:  StoredProcedure [dbo].[OrderWorkflow]    Script Date: 2/27/2017 9:55:12 PM ******/
SET ANSI_NULLS ON
GO
...
where o.dtDateStamp >= @dS and o.dtDateStamp <= @dE
...
select  
       ...
       --reports.ss.fn_TimeSpan(ol.dRecieved,ol.dFirstView) as 'xR2V_DHMS',
	    reports.ss.fn_TimeSpan(ol.dRecieved,ol.dFirstView) as 'xR2V_DHMS',
	   ...
	   case when datediff(mi,o.dtDateStamp,ol.dFirstView)>15 then 'N' else 'Y' end as 'Metric Met',
	  datediff(mi,o.dtDateStamp,ol.dFirstView) as 'MetricVariance',
       o.dtxScheduled as 'dtFirstScheduled', 
       us.sFull as 'sUserScheduled',
	  reports.ss.fn_TimeSpan(ol.dRecieved,o.dtxScheduled) as 'xR2S_DHMS',
	   case when datediff(mi,ol.dRecieved,o.dtxScheduled)>30 then 'N' else 'Y' end as 'Scheduling Metric Met',
	   datediff(mi,ol.dRecieved,o.dtxScheduled) as 'SchedulingMetricVariance'
from @tOrderList as ol
