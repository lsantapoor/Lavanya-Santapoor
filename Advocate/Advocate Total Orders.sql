...
DECLARE @iSiteRef int, @iDMEID int,@iCompanyRef int
DECLARE @iTZRef int
...
--set @iDMEID= (1639,336,1640)  --(Advocate DME ID)

--Create a permanent table to input data for every 20 days--
 IF OBJECT_ID('reports_bic.[dbo].[AdvocateTotalOrders]', 'U') IS NOT NULL 
  DROP TABLE reports_bic.[dbo].[AdvocateTotalOrders]; 

CREATE TABLE reports_bic.[dbo].[AdvocatetotalOrders](
...
insert into reports_bic.[dbo].[AdvocatetotalOrders]
SELECT	o.iOrderID,
		o.iPatientID,
		o.iDMEID,
		o.cStatusID,
...
--o.iReferenceOrderID,
	...
	left join reports.ss.tblLocation w on p.iClientRef=w.iLocationid where w.iCompanyref=1640  and 
o.dtcompleted between @StartDate and @EndDate 

	 --Below code emails the report to the recipients--
 Declare @query nvarchar(max);
    Set @query = 'select * from reports_bic.[dbo].[AdvocatetotalOrders]';
 ...
 Execute msdb.dbo.sp_send_dbmail
	 ...
	 ,@copy_recipients = 'lsantapoor@stateserv.com'
      ...
      , @query_attachment_filename = 'Total Orders Monthly.csv'
