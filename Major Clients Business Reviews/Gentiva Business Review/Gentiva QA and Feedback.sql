...
/*
  IF OBJECT_ID('reports_bic.dbo.tempregion', 'U') IS NOT NULL 
  DROP TABLE reports_bic.dbo.tempregion; 
create table  reports_bic.dbo.tempregion (
	[iHOS] [int] NULL,
	[iRgn] [int] NULL,
	[iTotal] [int] NULL,
	[iNoCnt] [int] NULL,
...
insert into  reports_bic.dbo.tempregion
select iHC_ID as iHOS, iSR_ID as iRgn, 
	iTotal, iNoCnt, iOpOut, iNoEng, iAbort, iUsable, iDone,
	xQ1, iQ1y, iQ1n, pQ1,
	xQ2, iQ2y, iQ2n, pQ2,
...
IF OBJECT_ID('reports_bic.dbo.Gentivaregion', 'U') IS NOT NULL 
  DROP TABLE reports_bic.dbo.Gentivaregion; 

create table  reports_bic.dbo.Gentivaregion (
	[seqno] [bigint] NULL,
	[iHOS] [int] NULL,
	[iRgn] [int] NULL,
	[iTotal] [int] NULL,
...
insert into  reports_bic.dbo.Gentivaregion
select ROW_NUMBER() over (order by Provider) as seqno,iHC_ID as iHOS, iSR_ID as iRgn, 
	iTotal, iNoCnt, iOpOut, iNoEng, iAbort, iUsable, iDone,
	xQ1, iQ1y, iQ1n, pQ1,
	xQ2, iQ2y, iQ2n, pQ2,
	xQ4, iQ4m, iQ4x, iQ4f, pQ4m, pQ4x, pQ4f,
	...
	Client, Provider,
	dtActualBeginDate,	dtActualEndDate 
--from reports_bic.dbo.tempregion
from @QA_Rollup
--get Notes

declare @regcnt as int,@rgn_step as int,@regionname char(100)

select @regcnt=count(*) from reports_bic.dbo.Gentivaregion

print 'region count is'
print @regcnt
  
 IF OBJECT_ID('reports_bic.dbo.GentivaQARegQtemp', 'U') IS NOT NULL 
  DROP TABLE reports_bic.dbo.GentivaQARegQtemp; 
Select * into reports_bic.dbo.GentivaQARegQtemp from reports_bic.dbo.GentivaQANationalQuestions where 0=1
...
declare @pq1 as numeric(9,6),@pq2 as numeric(9,6)declare @pQ4m as numeric(9,6), @pQ4x as numeric(9,6), @pQ4f as numeric(9,6)
declare @pQ5m as numeric(9,6), @pQ5x as numeric(9,6), @pQ5f as numeric(9,6)
declare @pQ6m as numeric(9,6), @pQ6x as numeric(9,6), @pQ6f as numeric(9,6)
declare @pQ7m as numeric(9,6), @pQ7x as numeric(9,6), @pQ7f as numeric(9,6)
declare @smonth char(12)
declare @providername char(100)

	set @smonth=DATENAME(month ,@sDate)
	select @providername=  Provider from reports_bic.dbo.Gentivaregion where seqno=@rgn_step


  select @pq1 = pQ1 from reports_bic.dbo.Gentivaregion where seqno=@rgn_step 
  select @pq2 = pQ2 from reports_bic.dbo.Gentivaregion where seqno=@rgn_step 
  select @pq4m = pQ4m from reports_bic.dbo.Gentivaregion where seqno=@rgn_step 
  select @pq4x = pQ4x from reports_bic.dbo.Gentivaregion where seqno=@rgn_step 
  select @pq4f = pQ4f from reports_bic.dbo.Gentivaregion where seqno=@rgn_step 
  select @pq5m = pQ5m from reports_bic.dbo.Gentivaregion where seqno=@rgn_step 
  select @pq5x = pQ5x from reports_bic.dbo.Gentivaregion where seqno=@rgn_step 
  select @pq5f = pQ5f from reports_bic.dbo.Gentivaregion where seqno=@rgn_step 
  select @pq6m = pQ6m from reports_bic.dbo.Gentivaregion where seqno=@rgn_step 
  select @pq6x = pQ6x from reports_bic.dbo.Gentivaregion where seqno=@rgn_step 
  select @pq6f = pQ6f from reports_bic.dbo.Gentivaregion where seqno=@rgn_step 
  select @pq7m = pQ7m from reports_bic.dbo.Gentivaregion where seqno=@rgn_step 
  select @pq7x = pQ7x from reports_bic.dbo.Gentivaregion where seqno=@rgn_step 
  select @pq7f = pQ7f from reports_bic.dbo.Gentivaregion where seqno=@rgn_step 

 

  insert into reports_bic.dbo.GentivaQARegQtemp values ('Equipment',@pq1,1-@pq1,null,@smonth,@providername);
  insert into reports_bic.dbo.GentivaQARegQtemp values ('Instruction',@pq2,1-@pq2,null,@smonth,@providername);
  insert into reports_bic.dbo.GentivaQARegQtemp values ('Delivery Person',@pQ4m, @pQ4f, @pQ4x,@smonth,@providername);
  insert into reports_bic.dbo.GentivaQARegQtemp values ('Phone Service',@pQ5m, @pQ5f, @pQ5x,@smonth,@providername);
  insert into reports_bic.dbo.GentivaQARegQtemp values ('Timely Delivery',@pQ6m, @pQ6f, @pQ6x,@smonth,@providername);
  insert into reports_bic.dbo.GentivaQARegQtemp values ('Timely Pickup',@pQ7m, @pQ7f, @pQ7x,@smonth,@providername);


set @rgn_step=@rgn_step+1
end

  insert into reports_bic.dbo.GentivaQANationalQuestions
  select * from reports_bic.dbo.GentivaQARegQtemp
...
declare @providername char(100)
  ...
  insert into reports_bic.dbo.GentivaQANationalQuestions values ('Equipment',@pq1,1-@pq1,null,@smonth,@providername);
  insert into reports_bic.dbo.GentivaQANationalQuestions values ('Instruction',@pq2,1-@pq2,null,@smonth,@providername);
  insert into reports_bic.dbo.GentivaQANationalQuestions values ('Delivery Person',@pQ4m, @pQ4f, @pQ4x,@smonth,@providername);
  insert into reports_bic.dbo.GentivaQANationalQuestions values ('Phone Service',@pQ5m, @pQ5f, @pQ5x,@smonth,@providername);
  insert into reports_bic.dbo.GentivaQANationalQuestions values ('Timely Delivery',@pQ6m, @pQ6f, @pQ6x,@smonth,@providername);
  insert into reports_bic.dbo.GentivaQANationalQuestions values ('Timely Pickup',@pQ7m, @pQ7f, @pQ7x,@smonth,@providername);
