...
declare @iDME int, @iHOS int, @iCID int, @iSID int
...
IF OBJECT_ID('reports_bic.dbo.iHOS_RedellistBizReview', 'U') IS NULL 
begin
declare @table_text char(100)
set @table_text = 'Hospice List Table Not present. Creating the table.... '
print @table_text

--  DROP TABLE reports_bic.dbo.iHOS_RedellistBizReview; 

create table reports_bic.dbo.iHOS_RedellistBizReview(
iHOS_SEQ_NO int,
iHOS_ID int,
...
--insert into reports_bic.dbo.iHOS_RedellistBizReview values(sequence_number,hospice id, Hospice Name)

insert into reports_bic.dbo.iHOS_RedellistBizReview values(1,10,'Optum Palliative and Hospice Care') --Optum Palliative and Hospice Care
insert into reports_bic.dbo.iHOS_RedellistBizReview values(2,12,'Alacare Home Health & Hospice')  --Alacare Home Health & Hospice
insert into reports_bic.dbo.iHOS_RedellistBizReview values(3,257,'Harbor Light Hospice') --Harbor Light Hospice
insert into reports_bic.dbo.iHOS_RedellistBizReview values(4,238,'SouthernCare, Inc {Curo}') --SouthernCare, Inc {Curo}
insert into reports_bic.dbo.iHOS_RedellistBizReview values(5,187,'Hospice of Santa Cruz County') --Hospice of Santa Cruz County
insert into reports_bic.dbo.iHOS_RedellistBizReview values(6,269,'Suncrest Health Services') --Suncrest Health Services
insert into reports_bic.dbo.iHOS_RedellistBizReview values(7,1057,'Sta-Home Health & Hospice, Inc.') --Sta-Home Health & Hospice, Inc.
insert into reports_bic.dbo.iHOS_RedellistBizReview values(8,120,'Banner Hospice') --Banner Hospice
insert into reports_bic.dbo.iHOS_RedellistBizReview values(9,232,'Hospice Partners of America') --Hospice Partners of America
insert into reports_bic.dbo.iHOS_RedellistBizReview values(10,295,'Encompass Home Health & Hospice') 
insert into reports_bic.dbo.iHOS_RedellistBizReview values(11,227,'Delaware Hospice')-- Delaware Hospice
insert into reports_bic.dbo.iHOS_RedellistBizReview values(12,144,'Signature Hospice')-- Signature Hospice 

end

IF OBJECT_ID('reports_bic.[dbo].[StandardRedelivery]', 'U') IS NOT NULL 
  DROP TABLE reports_bic.[dbo].[StandardRedelivery]; 


CREATE TABLE reports_bic.[dbo].[StandardRedelivery](
    smonth [varchar](12),
...
select @iHOScount= count(*) from reports_bic.dbo.iHOS_RedellistBizReview
print @iHOScount

select * from reports_bic.dbo.iHOS_RedellistBizReview
set @iHOS_step=1

while (@iHOS_step <= @iHOScount)
begin

select @iHOS=iHOS_ID from reports_bic.dbo.iHOS_RedellistBizReview where iHOS_SEQ_NO = @iHOS_step -- read Hospice id from the list 
select @iHOSNAME=iHOS_name from reports_bic.dbo.iHOS_RedellistBizReview where iHOS_SEQ_NO = @iHOS_step
-----------
...
\*-----------------------------------*/
	...
	insert into reports_bic.dbo.StandardRedelivery
select @smonth,rtrim(d.sName + ', ' + (s.sName)) as Provider, rtrim(c.sName) as Client, rtrim(isnull(re.sName,c.sState + ' Region')) as RegionName, --DATENAME(month ,pd.dComplete) as imonth
		rtrim(@iHOSNAME) as HospiceName, SUM(1) as [Cnt Orders], SUM(pd.iRedelivery) as [Cnt Redelivery], SUM(pd.iCntCSMissPair) as [Cnt Missed Pair],
	...
	join ss.tblUSState as us on us.sState = c.sState
	left join ss.tblSet as re on re.iSetId = c.iSetRef
	where dComplete >= @dS and dComplete <= @dE
	group by c.sName, d.sName + ', ' + s.sName,re.sName, c.sState + ' Region'
...
set @loop_step=@loop_step+1
end		--3 months loop ends	
set @iHOS_step=@iHOS_step+1
end --hospice loop end	
